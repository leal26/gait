function [xFINAL,Resi,exitflag,output,jacobian] = ContinuationFun(xINIT,x_Last,stepSize,numOPTS)
%UNTITLED Summary of this function goes here
%   Detailed explanation goes here

[xFINAL, constViol,exitflag,output,jacobian] = fsolve(@f_Continuation, xINIT, numOPTS);   
Resi = constViol'*constViol;
...
function residual = f_Continuation(X)
    % Compute residuals
    % Periodicity:
    ResiZF            = ZeroFunc_BipedApex(X); 
    residual          = ResiZF;
    residual(end+1)   = norm(x_Last - X) - stepSize;

end

function residual = ZeroFunc_BipedApex(X)

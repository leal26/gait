function [residual, T,Y,P,Y_EVENT,TE] = ZeroFunc_BipedApex(X)
...
%     residual(12)  = YL_TD(5) -  YR_TD(7); % Symmetrical
    ...
    function dydt_ = ode(~,y_)
        ...
        if contactL
            u = (1-y/cos(alphaL));
            k_NL = k*(.8*u^2-.8*u+1);
            Fx = Fx - (1- y/cos(alphaL))*k_NL*sin(alphaL);
            Fy = Fy + (1- y/cos(alphaL))*k_NL*cos(alphaL);
        end
        if contactR
            u = (1-y/cos(alphaR));
            k_NL = k*(.8*u^2-.8*u+1);
            Fx = Fx - (1- y/cos(alphaR))*k_NL*sin(alphaR);
            Fy = Fy + (1- y/cos(alphaR))*k_NL*cos(alphaR);
        end

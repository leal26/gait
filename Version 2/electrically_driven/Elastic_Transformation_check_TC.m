function [ sigma, T, eps_t, MVF, H_cur, Phi_fwd,Phi_rev, h, eps, chck ] = Elastic_Transformation_check_TC( P,TP,eps, eps_prev, t, t_prev, T_prev, T0, sigma_prev, Phi_fwd_prev, Phi_rev_prev, E, MVF, eps_t, eps_t_r, MVF_r, r, stress_flag )
...
while calculating
    ...
    % Flag to restrict stress if necessary
    if stress_flag
        if (sigma >= 0 || eps == eps_prev) %&& (r==0)
            calculating = false;
        else
            eps_prev_iteration = eps;
            eps = eps_prev + P.alpha*delta_T- sigma_prev/E;
            error = abs(eps - eps_prev_iteration);
            if error <= 10e-6
                calculating = false;
            end
        end
